...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default='/content/sequential_data', type=str or list,
                        help='dataset path')
    ...
    # online learning
    parser.add_argument('--sequential', default=False, type=bool)
    parser.add_argument('--num_samples', default=None, type=int)
    parser.add_argument('--path_model', default='', type=str)
    parser.add_argument('--online', action='store_false')
    parser.add_argument('--n_eval', default=1000, type=int)


    return parser

def resume_model(args):
    return(args.combined_model, )
...
def main(args):
    ...
    data_path = [args.data_path] if type(args.data_path) == str else args.data_path

    if args.num_samples:

        n_samples = args.num_samples//len(data_path)
        n_eval = args.n_eval//len(data_path)

        indices = np.random.randint(0, 50000, n_samples) 
        indices_eval = np.array(random.sample(list(set(range(50000)) - set(indices)), n_eval)) if args.online else None

        with open(os.path.join(args.output_dir, "indices.pkl"), "wb") as file:
            pickle.dump(indices, file )

        with open(os.path.join(args.output_dir, "indices_eval.pkl"), "wb") as file:
            pickle.dump(indices_eval , file )
    else:
        indices = None
    ...
    for path in os.listdir(data_path):
        ...
        dataset_train.append(tt_image_folder.ExtendedImageFolder(path, transform=transform_train, minimizer=indices, 
        ...
        dataset_val.append(tt_image_folder.ExtendedImageFolder(path, transform=transform_val, 
                                                            batch_size=1, minimizer=indices, 
                                                            single_crop=args.single_crop, start_index=0))
        

        dataset_eval.append(tt_image_folder.ExtendedImageFolder(path, transform=transform_val, 
                                                        batch_size=1, minimizer=indices_eval, 
                                                        single_crop=args.single_crop, start_index=0) )
